...
export default async function CustomerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      <Card><CardHeader><CardTitle className="text-base">Documents</CardTitle></CardHeader><CardContent className="space-y-2">{customer.documents.map((document) => { const viewAction = viewCustomerDocument.bind(null, customer.id, document.id); return <div key={document.id} className="flex items-center justify-between gap-3 rounded-lg border p-3"><div className="min-w-0"><p className="text-sm font-semibold capitalize">{document.documentType.replace("_", " ")}</p><p className="truncate text-xs text-muted-foreground">{document.fileName}{document.documentNumber ? ` · ${document.documentNumber}` : ""}</p></div><div className="flex items-center gap-2"><StatusBadge status={document.status} /><form action={viewAction}><Button type="submit" size="sm" variant="outline">View</Button></form></div></div>; })}{customer.documents.length === 0 && <p className="text-sm text-muted-foreground">No documents uploaded.</p>}</CardContent></Card>
