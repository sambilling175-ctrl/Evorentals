...
| D9-06 | P0 | Finalize returned rentals with immutable settlement snapshots and atomic closure | Blocked | Codex / `agent/d9-06-rental-settlement`; live schema has no payment, allocation, deposit, refund, invoice, or dues ledger; `rentals.total_amount` already includes quoted deposit, so paid/deposit/refund balances cannot be derived safely | D9-05, D10-01 | Rentals, Collections |
| D10-01 | P0 | Establish company-scoped receivables ledger for invoices, payments, allocations, deposits, refunds, and dues | In progress | Codex / `agent/d10-01-receivables-ledger`; draft PR #10; migrations/advisors verified; typed live workspace plus invoice/payment/refund commands and Zod server actions implemented; ambiguous PostgREST customer embeds removed after authenticated preview 500; validate passed; new preview smoke pending | D9-05, payment settings | Collections, Finance |
