...
| D4-04 | P1 | Add auth recovery and protected-route Playwright coverage | Blocked | Unassigned; route/invalid-token coverage can run without mail, but end-to-end recovery requires a stable SMTP/test mailbox or a Supabase admin-generated test link | Auth, tests |
...
| D11-03 | P1 | Dry-run and import conflict-free legacy customer metadata | Ready | Unassigned; import only the 13,762 clean rows after mapping/dry-run verification; D11-02 quarantines 30 rows; no KYC binaries in scope | D11-02 | Migration, customers |
