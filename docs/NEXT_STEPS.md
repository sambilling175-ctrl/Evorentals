...
| D9-06 | P0 | Finalize returned rentals with immutable settlement snapshots and atomic closure | Review | Codex / `agent/d9-06-rental-settlement`; migration `20260814052728_rental_settlements.sql` applied; invoker RPC, immutable snapshot, typed Zod action, and rental UI implemented; authenticated `/rentals` preview smoke renders the empty control board with zero console errors/warnings; isolated paid-settlement smoke passes with no persisted records; preview release remains pending | D9-05, D10-01 | Rentals, Collections |
| D10-01 | P0 | Establish company-scoped receivables ledger for invoices, payments, allocations, deposits, refunds, and dues | Review | Codex / `agent/d10-01-receivables-ledger`; draft PR #10; migrations/advisors verified; authenticated `/payments` preview renders ledger KPIs, posting forms, and empty invoice/collection states with zero console errors/warnings; rollback-only `supabase/tests/receivables_commands.sql` passed through the SECURITY INVOKER payment RPC and invalid-amount rejection, with zero persisted rows; browser financial command submission remains intentionally unexecuted | D9-05, payment settings | Collections, Finance |
...
| D11-01 | P1 | Stage and reconcile complete legacy customer metadata export without database writes | Review | Codex / `agent/d11-01-legacy-customer-staging`; commits `f3a8347`, `641fc89`, `91d83ae`; staging output and ID-only conflict manifest generated; `npm.cmd run validate` passed; no Supabase records or KYC binaries created; awaiting conflict-policy review | Complete customer CSV export | Migration, customers |
| D11-02 | P1 | Approve legacy customer conflict policy before import | Blocked | Unassigned; 10 source records have blank names and 20 rows share duplicate email/mobile identities; requires explicit quarantine/placeholder and duplicate-account policy | D11-01 | Migration, customers |
