...
| D12-06 | P1 | Add CI validation, Playwright route tests, migration checks, and SQL tests | Completed | Owner: Codex; branch `agent/d12-06-ci-hardening`; GitHub Actions workflow adds validate, 10-route Playwright, and Supabase artifact jobs; `npm.cmd run check:supabase`, `npm.cmd run test:e2e -- --workers=1 --reporter=line` (10 passed), and `npm.cmd run validate` passed; no records created | D12-04, D12-05 | CI, tests |
