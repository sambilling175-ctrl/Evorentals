...
| D12-03 | P0 | Review and integrate D10-03 returned-rental collections | Completed | Codex; PR #14 merge `8dceed3` is an ancestor of current `main`; re-review confirmed all four live migrations, SECURITY INVOKER/search-path boundary, company RLS, FK-index coverage, and `supabase/tests/returned_rental_collections.sql` rollback-only receipt/damage acceptance; no business records | D9-06, D10-01 | Collections, release |
...
| D12-04 | P0 | Integrate D4-05 RLS isolation and non-email Playwright coverage | In progress | D4-05 RLS integration merged as `c279387`; non-email Playwright route coverage remains; real-email recovery stays parked behind D4-04 | D4-05 | Release, tests |
...
| D12-06 | P1 | Add CI validation, Playwright route tests, migration checks, and SQL tests | Ready | No CI workflow exists yet; next implementation after D12-05 | D12-04, D12-05 | CI, tests |
