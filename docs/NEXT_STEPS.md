...
| D11-01 | P1 | Stage and reconcile complete legacy customer metadata export without database writes | Review | Codex / `agent/d11-01-legacy-customer-staging`; complete CSV staged locally; ID-only conflict manifest reviewed; no Supabase records or KYC binaries created | Complete customer CSV export | Migration, customers |
...
| D11-03 | P1 | Dry-run and import conflict-free legacy customer metadata | In progress | Codex / `agent/d11-03-customer-import-next`; local reconciliation and authenticated remote dry-run passed for 13,760 rows across 69 chunks; checksum `6511d29d20dca127fa75f0324b43cc62046090ee7d64f2c5af6b8e438422728c`; `applied=false`, zero import rows; explicit checksum approval remains before `--apply` | D11-02 | Migration, customers |
