...
| D10-03 | P0 | Record returned-rental collections with damage allocations and immutable receipt/audit history | Review | Codex / `agent/d10-03-returned-rental-collections`; commits `1621c72`, `45be7f1`; migration applied as live version `20260814060344`; invoker/RLS/grants/triggers verified; advisors show only pre-existing findings; `/payments` force-dynamic route and retry boundary added after an auth-refresh race; authenticated empty-ledger smoke passes; authorized transaction smoke pending | D9-05, D10-01 | Collections, settlement |
