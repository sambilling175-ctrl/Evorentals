...
| D13-03 | P1 | Add vehicle intake inspection with odometer and battery capture | Review | Codex / `agent/d13-03-vehicle-intake-inspection`; draft PR #22; migrations `20260821145735_d13_03_vehicle_intake_inspection` and `20260821145849_d13_03_require_intake_before_inspection` applied; immutable intake history, stale-odometer guard, atomic bike telemetry reconciliation, and inspection-stage gate; local validate/checks and 11-route Playwright passed; CI run `32496034939` passed; dashboard count-query hardening commit `cc89d1e`; latest Vercel preview READY at `https://evorentals-3adi0a5t6-wephotons1.vercel.app`; stacked on D13-02 review branch; no intake records | D13-02 | Service, fleet |
| D13-04 | P1 | Assign service job cards to internal employees or external garages | Review | Codex / `agent/d13-04-service-job-card-assignments`; commits `48c4bc0`, `2444d8d`; migration `20260821160000_d13_04_service_job_card_assignments` applied; append-only assignment history, company-scoped RLS, invoker RPC/trigger guards, active-employee and external-garage validation, bounded auth-refresh retry, service workspace/action UI, rollback-only SQL artifact, advisors, `check:supabase`, and `validate` passed; draft PR #23; Vercel preview READY at `https://evorentals-h3jg9mvnl-wephotons1.vercel.app`; no service business records | D13-03 | Service, maintenance |
| D13-05 | P1 | Verify and harden the controlled service pipeline | Review | Codex / `agent/d13-05-service-pipeline-verification`; D13-02 already provides the invoker transition RPC and immutable status-event history for `requested -> inspection -> in_service -> waiting_parts -> qc -> completed`; D13-03 intake gate and D13-04 assignment dependency confirmed; added rollback-only `supabase/tests/d13-05-service-pipeline.sql`; fixed authenticated dashboard 500 caused by a transient `bikes_total` HEAD count 401 by using bounded exact-count GETs; live verification and `validate` passed; no migration or business records | D13-04 | Service, maintenance |
| D13-06 | P1 | Atomically update fleet availability when service begins or finishes | Review | Codex / `agent/d13-06-service-fleet-availability`; migration `20260821165156_d13_06_service_fleet_availability` applied; invoker fixed-search-path trigger locks the bike, rejects retired/actively rented vehicles at service start, sets `maintenance`, and releases non-retired vehicles to `available` on completion; rollback-only SQL test and live catalog verification passed; no service business records | D13-05, D7-01 | Service, fleet |
| D13-07 | P1 | Add service dashboard metrics, turnaround time, overdue jobs, and readiness | Review | Codex / `agent/d13-07-service-dashboard`; commit `b2f795e`; draft PR #26; READY preview `https://evorentals-f0z8dyh44-wephotons1.vercel.app`; typed live stage counts, SLA-based overdue jobs, average turnaround, and availability-backed deployment readiness; `npm.cmd run validate` and `check:supabase` passed; live service job-card count remains zero; no migration or business records | D13-06 | Service, reporting |
| D13-08 | P1 | Verify service RLS/history/advisors and add rollback-only SQL tests | Review | Codex / `agent/d13-08-service-security-tests`; draft PR #27, migration `20260822060115_d13_08_service_security_hardening` applied, READY branch preview `https://evorentals-git-agent-d13-08-service-security-tests-wephotons1.vercel.app`; live rollback-only security/history contract passed with zero records; local artifact/validate checks passed; advisors show only pre-existing project-wide findings | D13-07 | Service, security, tests |
| D14-01 | P1 | Add vendor and garage directory | Review | Codex / `agent/d14-01-service-vendor-directory`; draft PR #28; migration `20260822060840_d14_01_service_vendor_directory` applied; company-scoped `service_vendors` RLS, authenticated invoker RPCs with fixed empty search path, typed service/actions, and `/service/vendors` UI implemented; live rollback-only contract, validation, advisors, CI, and Vercel preview `https://evorentals-git-agent-d14-01-service-vendor-directory-wephotons1.vercel.app` passed; no directory records created | D13-08 | Service, vendors |
| D14-02 | P1 | Add spare-parts catalogue and stock movements | Review | Codex / `agent/d14-02-service-parts-stock`; draft PR #29; migrations `20260822062423_d14_02_service_parts_stock` and `20260822063200_d14_02_service_parts_hardening` applied; company-scoped catalogue and append-only stock ledger use guarded authenticated invoker RPCs, fixed empty search paths, FK indexes, typed service/actions, and `/service/parts` UI; live rollback-only contract, validation, advisors, CI, and Vercel preview `https://evorentals-git-agent-d14-02-service-parts-stock-wephotons1.vercel.app` passed; no part or stock records created | D14-01 | Service, inventory |
