...
| D13-04 | P1 | Assign service job cards to internal employees or external garages | Review | Codex / `agent/d13-04-service-job-card-assignments`; commit `48c4bc0`; migration `20260821160000_d13_04_service_job_card_assignments` applied; append-only assignment history, company-scoped RLS, invoker RPC/trigger guards, active-employee and external-garage validation, service workspace/action UI, rollback-only SQL artifact, advisors, `check:supabase`, and `validate` passed; draft PR #23; Vercel preview READY at `https://evorentals-h3jg9mvnl-wephotons1.vercel.app`; no service business records | D13-03 | Service, maintenance |
