...
| D14-02 | P1 | Add spare-parts catalogue and stock movements | Review | Codex / `agent/d14-02-service-parts-stock`; migrations `20260822062423_d14_02_service_parts_stock` and `20260822063200_d14_02_service_parts_hardening` applied; company-scoped catalogue and append-only stock ledger use guarded authenticated invoker RPCs, fixed empty search paths, FK indexes, typed service/actions, and `/service/parts` UI; live rollback-only contract, validation, advisors, and artifact checks passed; no part or stock records created; preview handoff pending | D14-01 | Service, inventory |
