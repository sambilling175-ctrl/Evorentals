...
| D14-01 | P1 | Add vendor and garage directory | Review | Codex / `agent/d14-01-service-vendor-directory`; migration `20260822060840_d14_01_service_vendor_directory` applied; company-scoped `service_vendors` RLS, authenticated invoker RPCs with fixed empty search path, typed service/actions, and `/service/vendors` UI implemented; live rollback-only contract, validation, and advisors passed; no directory records created; preview handoff pending | D13-08 | Service, vendors |
