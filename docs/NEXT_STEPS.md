...
| D9-06 | P0 | Finalize returned rentals with immutable settlement snapshots and atomic closure | Review | Codex / `agent/d9-06-rental-settlement`; migration `20260814052728_rental_settlements.sql` applied; invoker RPC, immutable snapshot, typed Zod action, and rental UI implemented; validate passed; authenticated preview smoke passed for `/rentals` and `/payments` with zero records, zero browser console errors, and no current Vercel runtime errors; mutation smoke remains pending because no production returned rental/invoice exists | D9-05, D10-01 | Rentals, Collections |
...
| D10-02 | P1 | Replace reports placeholder with live rental, fleet, customer, and collections summaries plus CSV export | Review | Codex / `agent/d10-02-live-reports`; commit `d6de2b2`; `npm.cmd run validate` passed; preview serves `/reports` and redirects unauthenticated users to login; no runtime errors; authenticated smoke pending | D9-06, D10-01 | Reports, dashboard |
| D10-03 | P0 | Record returned-rental collections with damage allocations and immutable receipt/audit history | Review | Codex / `agent/d10-03-returned-rental-collections`; commits `1621c72`, `45be7f1`, `9b54cc8`; migration applied as live version `20260814060344`; invoker/RLS/grants/triggers verified; advisors show only pre-existing findings; `/payments` force-dynamic route and retry boundary added after an auth-refresh race; authenticated empty-ledger smoke passes on READY deployment `dpl_BBP2ft9ThS8iHTiXwCCBBYXTudss`; authorized transaction smoke pending | D9-05, D10-01 | Collections, settlement |
