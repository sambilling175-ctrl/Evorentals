...
| D11-01 | P1 | Stage and reconcile complete legacy customer metadata export without database writes | In progress | Codex / `agent/d11-01-legacy-customer-staging`; local CSV validator writes only git-ignored staging output; no Supabase records or KYC binaries created | Complete customer CSV export | Migration, customers |
