...
| D10-01 | P0 | Establish company-scoped receivables ledger for invoices, payments, allocations, deposits, refunds, and dues | Review | Codex / `agent/d10-01-receivables-ledger`; draft PR #10; migrations/advisors verified; authenticated `/payments` preview renders ledger KPIs, posting forms, and empty invoice/collection states with zero console errors/warnings; financial command smoke remains intentionally unexecuted because it requires non-production test records | D9-05, payment settings | Collections, Finance |
